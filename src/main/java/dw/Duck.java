...
public class Duck implements DuckConstants, java.io.Serializable {
	...
	public Duck(ScannedRobotEvent inevt, long timeCaptured, double myX, double myY, double myHeading, double myVelocity,
			boolean inalive, boolean inlocked) {
		setInfo(inevt);
	...
	public void setInfo(ScannedRobotEvent inevt) {
		...
		duckInfo = new DuckInfo(inevt.getName(), inevt.getEnergy(), inevt.getBearing(), inevt.getDistance(),
				inevt.getHeading(), inevt.getVelocity());
	...
	private void calcDuckXY() {
		...
		// System.out.println( duckInfo.getName() + " is at " + duckX + ", " +
		// duckY);
	...
	public boolean equals(Object obj) {
		if (obj instanceof Duck) {
			// System.out.println("in obj is " + ((Duck) obj).getName() + " me
			// is " + duckInfo.getName());
	...
	class DuckInfo implements java.io.Serializable {
		...
		public DuckInfo(String inname, double inenergy, double inbearing, double indistance, double inheading,
				double invelocity) {
			setName(inname);
			setEnergy(inenergy);
